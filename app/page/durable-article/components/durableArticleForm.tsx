...
/* ----------------------------ข้อมูลตัวอย่าง------------------------------------ */
  ...
  const handleAutoFill = () => {
    // A. เตรียมชุดข้อมูล
    const categories = [
      "ครุภัณฑ์วิทยาศาสตร์การแพทย์", "ครุภัณฑ์สำนักงาน", 
      "ครุภัณฑ์คอมพิวเตอร์", "ครุภัณฑ์การแพทย์", "ครุภัณฑ์ไฟฟ้าและวิทยุ"
    ];
    const descriptions = [
      "เครื่องวัดความดันโลหิตแบบดิจิตอล Omron HEM-7120",
      "เครื่องผลิตออกซิเจน ขนาด 5 ลิตร Yuwell รุ่น 8F-5AW",
      "เตียงผู้ป่วยไฟฟ้า 3 ฟังก์ชั่น พร้อมที่นอน",
      "เครื่องตรวจคลื่นไฟฟ้าหัวใจ (EKG) 12 ลีด",
      "กล้องส่องตรวจหู คอ จมูก (Otoscope Set)"
    ];
    const vendors = ["บจก. เครื่องมือแพทย์ไทย", "ร้านเมดิคอล ซัพพลาย", "หจก. ตากเวชภัณฑ์", "บริษัท นำเข้าจำกัด"];
    ...
    const randPrice = getRandomInt(5000, 50000);
    const randLifespan = getRandomElement([3, 5, 8, 10]);
    const randDate = dayjs().subtract(getRandomInt(1, 12), "month"); // ย้อนหลัง 1-12 เดือน

    // C. คำนวณค่าเสื่อมเบื้องต้น (ราคา / (ปี * 12))
    ...
    form.setFieldsValue({
      ...
      attributes: "สภาพใหม่ 100%, มีใบรับประกันสินค้า",
